const express = require("express");
const cors = require("cors");
const app = express();
require("dotenv").config();
const { MongoClient, ServerApiVersion, ObjectId } = require("mongodb");
const port = process.env.PORT || 5000;

// midlleware
app.use(cors());
app.use(express.json());

// cofeeMAster
// pVqX7ccr0eNSfRU7
console.log(process.env.DB_USER);

const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.dlekrip.mongodb.net/?retryWrites=true&w=majority`;

// Create a MongoClient with a MongoClientOptions object to set the Stable API version
const client = new MongoClient(uri, {
  serverApi: {
    version: ServerApiVersion.v1,
    strict: true,
    deprecationErrors: true,
  },
});

async function run() {
  try {
    // Connect the client to the server	(optional starting in v4.7)
    await client.connect();

    const cofeeCollection = client.db("cofeeDB").collection("cofee");

    app.get("/cofee", async (req, res) => {
      const cursor = cofeeCollection.find();
      const result = await cursor.toArray();
      res.send(result);
    });

    app.get("/cofee/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
      const result = await cofeeCollection.findOne(query);
      res.send(result);
    });

    app.post("/cofee", async (req, res) => {
      const newCofee = req.body;
      console.log(newCofee);
      // insert
      const result = await cofeeCollection.insertOne(newCofee);
      res.send(result);
    });

    app.put("/cofee/:id", async (req, res) => {
      const id = req.params.id;
      const filter = { _id: new ObjectId(id) };
      const options = { upsert: true };
      const UpdatedCoffe = req.body;
      const Coffe = {
        $set: {
          name: UpdatedCoffe.name,
          designation: UpdatedCoffe.designation,
          category: UpdatedCoffe.category,
          details: UpdatedCoffe.details,
          photo : UpdatedCoffe.photo
        },
      };
      const result = await cofeeCollection.updateOne(filter, Coffe,options);
      res.send(result);
    });

    app.delete("/cofee/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
      const result = await cofeeCollection.deleteOne(query);
      res.send(result);
    });

    // Send a ping to confirm a successful connection
    await client.db("admin").command({ ping: 1 });
    console.log(
      "Pinged your deployment. You successfully connected to MongoDB!"
    );
  } finally {
    // Ensures that the client will close when you finish/error
    // await client.close();
  }
}
run().catch(console.dir);

app.get("/", (req, res) => {
  res.send("server is running on port");
});
app.listen(port, () => {
  console.log(`server is running on port: ${port}`);
});
